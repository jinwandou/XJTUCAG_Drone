    ...
    if (cam_image) {
        {
            ...
            if(img_debug)
            {
              cv::imshow("origin",cam_image_copy);
              cv::waitKey(5);
            }
...
int main(int argc, char **argv)
{
    ...
    std::string ros_path = ros::package::getPath("px4_autonomy");
    ...
    YAML::Node camera_config = YAML::LoadFile(ros_path + "/src/perception/object_detection/config/" + camera_info);
    // 相机内部参数
    ...
    while (ros::ok())
    {
        ...
        if (markerids.size() > 0)
        {
            ...
            if (-1 != t)
            {
                ...
                ROS_INFO("YAW_error: %f",eulerVec(0));
