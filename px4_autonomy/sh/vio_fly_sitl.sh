...
gnome-terminal --window -e 'bash -c "source /opt/ros/kinetic/setup.bash;source ~/project/px4_ws/devel/setup.bash;roscore; exec bash"' \
--tab -e 'bash -c "sleep 10; cd ~/PX4_Firmware;roslaunch px4 posix_sitl.launch; exec bash"' \
--tab -e 'bash -c "sleep 10; source ~/project/px4_ws/devel/setup.bash;roslaunch mavros px4.launch fcu_url:="udp://:14540@127.0.0.1:14557" gcs_url:=udp-b://@; exec bash"' \
--tab -e 'bash -c "sleep 5; source ~/project/px4_ws/devel/setup.bash;roslaunch px4_command px4_pos_estimator.launch; exec bash"' \
--tab -e 'bash -c "sleep 5; source ~/project/px4_ws/devel/setup.bash;roslaunch px4_command px4_pos_controller.launch; exec bash"' \
--tab -e 'bash -c "sleep 5; source ~/project/px4_ws/devel/setup.bash;rosrun px4_command move; exec bash"' \
