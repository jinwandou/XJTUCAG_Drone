...
void DroneControl::det_position_cb(const prometheus_msgs::DetectionInfo::ConstPtr &msg)
{
  prometheus_msgs::DetectionInfo Detection_info = *msg;
  if(!Detection_info.detected)
    return;
  ROS_INFO("det detectted!");
  det_position.pose.position.x = Detection_info.position[0];
  det_position.pose.position.y = Detection_info.position[1];
  det_position.pose.position.z = Detection_info.position[2];
//  marker_position_.poses[0].position.x = Detection_info.position[0];
//  marker_position_.poses[0].position.y = Detection_info.position[1];
//  marker_position_.poses[0].position.z = Detection_info.position[2];
//  marker_position_.poses[0].orientation = Detection_info.yaw_error
  static int cnt = 0;
  static tf2_ros::TransformBroadcaster br;

  // Transformation from drone to visual marker
  transformStamped_.header.stamp = Detection_info.header.stamp; 
...
//  transformStamped_.transform.translation.z = -marker_position_.poses[0].position.y;
  ...
  transformStamped_.transform.translation.x = Detection_info.position[2];
  transformStamped_.transform.translation.y = -Detection_info.position[0];
  transformStamped_.transform.translation.z = -Detection_info.position[1];
  if(USE_MARKER_ORIENTATION)
  {
    if(marker_position_.poses.size()){
      ...
      //else if(yaw > 3*M_PI/2) yaw -= 2*M_PI;
      transformStamped_.transform.rotation = tf::createQuaternionMsgFromYaw(Detection_info.yaw_error);
    }
  }
  else
  {
    
    double rad = -atan2f(Detection_info.position[0], Detection_info.position[2]);
    transformStamped_.transform.rotation = tf::createQuaternionMsgFromYaw(rad);
  }
  br.sendTransform(transformStamped_);

  double target_distance = Detection_info.position[2]/4; // Target distance is proportional to horizontal distance
...
void DroneControl::takeOff()
{
  ...
  while(ros::ok() && local_position_.pose.position.z < (TAKEOFF_ALTITUDE - 0.1))
  {
    ros_client_->setpoint_pos_pub_.publish(setpoint_pos_ENU_);
    ros::spinOnce();
    rate_->sleep();
...
void DroneControl::scanBuilding()
{
  ...
  //fly forward 15m
//  for(uint i = 0; i < 4; i++){
//    current_endpoint.pose.position.x += 5.0*cos(yaw);
//    current_endpoint.pose.position.y += 5.0*sin(yaw);
//    current_endpoint.pose.position.z = 3;
//    scanUntil(current_endpoint);
//  }
  current_endpoint.pose.position.x += 2.0*cos(yaw);
  current_endpoint.pose.position.y += 2.0*sin(yaw);
  current_endpoint.pose.position.z = 4;
  scanUntil(current_endpoint);
  //Fly down
  current_endpoint.pose.position.z = 3;
  scanUntil(current_endpoint);
...
//  scanUntil(current_endpoint);
  ...
  //Fly 4 m to the left
  current_endpoint.pose.position.x -= 3*sin(yaw);
  current_endpoint.pose.position.y += 3*cos(yaw);
  scanUntil(current_endpoint);
...
void DroneControl::centerMarker()
{
  ...
  int i, cnt = 0;
  double yaw = currentYaw();
//  if(marker_position_.poses.size() == 0)
//    return;
  double verticalDistance = det_position.pose.position.z;
