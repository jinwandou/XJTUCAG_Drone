...
void depth_cam_cb(const sensor_msgs::Image::ConstPtr& msg)
{
  if(!encodingInitialized)
  {
    ...
    ROS_INFO("encodingInitialized: %d, ringbufferActive: %d",encodingInitialized,ringbufferActive);
  }
  if(ringbufferActive)
  {
...
//    static const float cy = 179.5;
    ...
    // iris_depth_camera stil model param
    static const float fx = 56.560089527741965;
    static const float fy = 56.560089527741965;
    static const float cx = 32.5;
    static const float cy = 24.5;
...
{
  ROS_INFO("trajectory planner node");
  ros::init(argc, argv, "collision_avoid",ros::init_options::NoSigintHandler);
  ros::NodeHandle nh;
