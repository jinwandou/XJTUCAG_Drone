...
int main(int argc, char **argv) {
  ...
  std::vector<Eigen::Vector3d> p = {Eigen::Vector3d(-5, -5, 0),Eigen::Vector3d(-5, -4, 0),Eigen::Vector3d(-4, -4, 0),
                                    Eigen::Vector3d(-3, -4, 0),Eigen::Vector3d(0, -5, 0),Eigen::Vector3d(2, 2, 0),
                                    Eigen::Vector3d(-5, 4, 0)};
  
  ewok::UniformBSpline3DOptimization<6> spline_opt(start_point, 0.5);

  for (int i = 0; i < num_points; i++) {
    spline_opt.addControlPoint(p[i]);
  }
